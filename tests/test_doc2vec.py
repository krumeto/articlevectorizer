from articlevectorizer.vectorize.doc2vecvectorizer import (
    Doc2VecVectorizer,
)
import pytest
import gensim
...
load_dotenv()
pinsboard = "ft-data-science-prod/datasets/article-vectorisation"
board = pins.board_s3(pinsboard)

train = board.pin_read("articles_train").assign(small_text = lambda d: d['title'] + d["summary"])
...
def testPersistence(model = model, input = similar_input):
    ...
    passed = np.allclose(sorted(loaded_transformed_vecs), sorted(original_transformed_vecs), atol=1e-1)
    assert passed == True 
...
def testConsistencyWithGensimModel(input = input, similar_input = similar_input):
    ...
    gensim_d2vmodel = gensim.models.Doc2Vec(d2v_sentences, min_count=1, vector_size=100, dm = 1, epochs = 5)
    ...
    vec_gensim_model = gensim_d2vmodel.infer_vector(gensim.utils.simple_preprocess(doc))  # vector returned by Doc2Vec
